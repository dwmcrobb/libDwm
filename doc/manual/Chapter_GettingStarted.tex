...
listing~\ref{listing:example06_postIO} adds support for
\hyperref[section:DwmStreamIOClass]{\texttt{Dwm::StreamIO}} by adding the
\texttt{Read(std::istream \&)} and \texttt{Write(std::ostream \&) const}
member functions for the case where C++26 reflection is not available
(\texttt{DWM\_CAN\_USE\_REFLECTION} is not defined).  Note that these
member functions are simple in this case because our member data types
are directly supported by
\hyperref[section:DwmStreamIOClass]{\texttt{Dwm::StreamIO}} and hence we
can use the \texttt{ReadV()} and \texttt{WriteV()} members of
\hyperref[section:DwmStreamIOClass]{\texttt{Dwm::StreamIO}} to implement
these members.
